...
export function Uploader({ value, onChange, fieldTypeAccepted }: iAppProps) {
    ...
    const uploadFile = useCallback(async (file: File) => {
        ...
        try {
            const presignedReqponse = await fetch("/api/s3/upload", {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    size: file.size,
                    isImage: fieldTypeAccepted === "image"
                })
            })

            if (!presignedReqponse.ok) {
            ...
            const { presignedUrl, key } = await presignedReqponse.json()
    ...
    const { getRootProps, getInputProps, isDragActive } = useDropzone({
        onDrop,
        accept: fieldTypeAccepted === "video" ? { "video/*": [] } : { "image/*": [] },
        maxFiles: 1,
        multiple: false,
        maxSize: fieldTypeAccepted === "image" ? 5 * 1024 * 1024 : 5000 * 1024 * 1024,
